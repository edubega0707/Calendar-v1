...
class Cregistro extends CI_controller
{
	...
	{ 
		...
		if ($this->form_validation->run())
		{
			...
			$param['correo_usuario']=$this->input->post('mail_usuario');
	...
	{
		$this->load->library('form_validation');
		$this->form_validation->set_rules('correo_usuario', 'Correo del usuario', 'required|trim|min_length[2]|max_length[150]',
		...
		if($this->form_validation->run())
		{
			$username=$this->input->post('correo_usuario');
...

	{
		...
		{	
			...
			$data['usuario']=$this->Mregistro->getasesor($data['session']);			
